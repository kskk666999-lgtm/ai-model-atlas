const SECTIONS: { title: string; body: React.ReactNode }[] = [
  ...
  },
  {
    ...
    body: (
      <ul className="list-disc space-y-2 pl-5">
        <li>所有来源的模型名先经过<b>显式别名表</b>映射到注册表 canonical_id。没有命中别名的名称不会丢弃，也不会被猜测合并——它们以来源原始名称出现在官方原始榜上，并写入 unmapped-models.json 等待人工补充别名。</li>
...
4. overall_index(m)
     ...
     仅统计该模型有数据的 c，缺失能力不按 0 分计入。`}</pre>
        <p>综合榜准入门槛：模型需覆盖 <b>≥4 个能力类别、≥5 个基准、≥2 个独立数据来源</b>，否则不进入默认综合榜。</p>
        <p>能力只有一个可靠基准时，官方原始榜照常展示，综合指数标注「单一来源」，不伪装成多源共识。</p>
      </div>
    ),
  },
  {
  ...
  },
  {
    ...
    body: (
      <ul className="list-disc space-y-2 pl-5">
        ...
        <li>未映射的模型名只出现在官方原始榜中，不参与跨来源综合，直到人工确认其身份。</li>
      </ul>
    ),
  },
];
